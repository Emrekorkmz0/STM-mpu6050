...
int main(void)
{
  ...
  for(i=0;i<1024;i++){
	  //TestingSignal[i]=(float)((400*sin(2*3.14*10*i*0.001)))+512;
	  TestingSignal[i]=(float)((400*sin(2*3.14*10*i*0.001)))+(100*sin(2*3.14*400*i*0.001))+512;
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  for(i=100;i<900;i++){
		  OutputSignal[i]=iir(TestingSignal[i]);
		  HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1,DAC_ALIGN_12B_R,(int)(OutputSignal[i]));
		  HAL_DAC_SetValue(&hdac, DAC_CHANNEL_2, DAC_ALIGN_12B_R, (int)(TestingSignal[i]));
		  HAL_Delay(0);
	  }
